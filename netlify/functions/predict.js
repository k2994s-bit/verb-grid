...
exports.handler = async (event) => {
  ...
  const force = (payload.force || "2").toString().slice(0, 2);
  ...
  const system = [
    ...
    "The framework has three layers: casino (variable-reward compulsion), congregation (parasocial bonds), and genuine person-to-person connection.",
    ...
    "Guardrail, the puritan trap: never destroy genuine connection while targeting the casino.",
    "A verb is an interface intervention. It has a STRENGTH from 1 (gentle) to 3 (strong), and acts on a concrete UI surface.",
    "Important: strength is not neutral. The friction and restriction literature shows a sweet spot. Too gentle (1) and the move is ignored or habituated to. The middle (2) is where behaviour actually shifts. Too strong (3) and it is circumvented, abandoned, or it damages genuine connection. The tool computes the strength-dependent journey itself, so you only describe the verb's identity, not a fixed journey.",
    "Return ONLY valid JSON, no markdown, no preamble, with exactly these keys:",
    '{"def": string (one clear plain sentence saying what the verb does to the user), "mech": string (a short gerund phrase naming the mechanism, e.g. "silencing the broadcast alerts"), "anchor": string (the realistic good outcome at a MODERATE strength, grounded, one clause, cite a real study in parentheses if apt), "layer": string, "principle": string, "guard": string (the connection check, what it must not touch), "basis": string (closest real study or pattern)}',
    "Keep every field short. Do not wrap strings in extra quotes."
  ].join(" ");

  const user = `Verb: ${name}. Strength: ${force} of 3. UI surface: ${surface}.${desc ? " Intent: " + desc : ""} Describe it as specified.`;
